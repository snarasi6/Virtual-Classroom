...
txtMessage=new JTextArea(5,80);
usersList=new JList();

sendButton=new JButton("Post");
logoutButton=new JButton("Leave");
loginButton=new JButton("Attend");
exitButton=new JButton("Exit");

JPanel center1=new JPanel();
center1.setLayout(new BorderLayout());
center1.add(new JLabel("White Board",JLabel.CENTER),"North");
...
chatWindow.pack();
chatWindow.setTitle("Enter first name for Class");
...
if(s!=null)
{
JOptionPane.showMessageDialog(chatWindow,"you will be left class right now. ","Exit",JOptionPane.INFORMATION_MESSAGE);
logoutSession();
}
System.exit(0);
}
...
{
if(s==null)
 	{JOptionPane.showMessageDialog(chatWindow,"Please Enter First name"); return;}
try{
	dos.writeUTF(txtMessage.getText());
	txtMessage.setText("");
     }
catch(Exception excp){txtBroadcast.append("\nattend button click :"+excp);}
}
if(temp==loginButton)
{
    
   
String uname=JOptionPane.showInputDialog(chatWindow,"Enter Your First Name: ");
if(uname!=null)
	clientChat(uname); 
...
try{
     ...
     t1.start();
     dos.writeUTF(uname);
     chatWindow.setTitle(uname+" Class Window");
    }
