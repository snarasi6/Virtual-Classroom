...
import java.sql.*;
class fregister extends JFrame implements ActionListener
{
	JFrame f1;
	JPanel p1;
	JButton b1,b2,b3;
	...
	fregister(String t,String u)
	{
		...
		f1=new JFrame(" Faculty Registration");
		p1=new JPanel();
		...
		l0=new JLabel("FACULTY  REGISTRATION  FORM  ");	l11=new JLabel("");
		...
		{
			b1.setText("Modify");
			l0.setText("FACULTY MODIFICATION FORM  ");
			modify(uu);
		}
...
"jdbc:oracle:thin:@localhost:1521:xe","system","abi");  
			PreparedStatement pst=con.prepareStatement("insert into t_account values(?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			int rs=pst.executeUpdate();
                        new fSendMail().sendMail(null, null, null,t3.getText());
			if(rs==0)
			{
				return false;
			}
...
"jdbc:oracle:thin:@localhost:1521:xe","system","abi");
			...
			j5.setSelectedItem(rs.getString("qual"));
			t6.setEnabled(false);
		}
	}
...
"jdbc:oracle:thin:@localhost:1521:xe","system","abi");
			...
			PreparedStatement pst1=con.prepareStatement("delete from t_account where uname='"+u+"'");
